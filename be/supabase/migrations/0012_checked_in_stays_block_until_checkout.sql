-- A planned checkout is an estimate. A checked-in guest owns the room until
-- the explicit checkout transaction changes the reservation status. This
-- migration makes that rule authoritative for every new intake operation.
...
begin
  ...
  -- Do not use planned_check_out_at here. Until staff performs checkout,
  -- there is still a guest in the room and both immediate stays and future
  -- reservations for this physical room must be rejected.
  if exists (
    select 1
    from public.reservations existing_stay
    where existing_stay.room_id = p_room_id
      and existing_stay.status = 'checked_in'
  ) then
    raise exception using errcode = 'P0001', message = 'Room is occupied until checkout is completed';
  end if;
...
-- `/reservations` remains an authenticated public API. Protect that legacy
-- draft-create path as well so no API can bypass the intake function above.
...
drop trigger if exists reservations_prevent_occupied_room on public.reservations;
create trigger reservations_prevent_occupied_room
before insert or update of room_id, status on public.reservations
for each row execute function public.prevent_reservation_on_occupied_room();
